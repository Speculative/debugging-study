...
class APIGateway:
    ...
    def register_user(self, user_id: str, api_key: str, quota: int):
        ...
        self._auth_service.register(user_id, api_key)
        self._quota_service.set_quota(user_id, quota)

    def handle_request(self, request: Request) -> Response:
        """Process an incoming API request through the gateway pipeline."""
        # Step 1: Authenticate
        authenticated_user = self._auth_service.validate(request.api_key)
        if authenticated_user is None:
            return self._finalize(request, 401, reason="unauthorized")

        # Step 2: Check quota
        quota_remaining = self._quota_service.get_remaining(request.user_id)
    ...
    def _finalize(self, request, status, body=None, reason=None, quota_remaining=None):
